...
if (downloadPupil)
    ...
    for ii = 1:numel(analyses)
        ...
        if ~contains(analyses{ii}.label, 'hcp-func') || ~contains(analyses{ii}.label, runName)
            analyses{ii} = [];
        end
    end
    
    analyses = analyses(~cellfun('isempty', analyses));
    session = fw.getSession(analyses{1}.parents.session);
    
    if strcmp(session.label, 'Session 1') || strcmp(session.label, 'Session 1a') || strcmp(session.label, 'Session 1b')
        sessionName = 'session1_restAndStructure';
    elseif  strcmp(session.label, 'Session 2')
...
if (downloadStructural)
    destinationOfStructuralScan = fullfile(anatDir, 'T1w_acpc_dc_restore.nii.gz');
