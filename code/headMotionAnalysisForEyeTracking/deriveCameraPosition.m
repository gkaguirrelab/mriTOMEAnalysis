...
%
% Description:
%   The purpose of this routine is to produce a vector that describes the
%   position of a camera, relative to the front surface of the eye, during
%   an MRI scan. The routine makes use of the "_Movement_Regressors.txt"
%   file that is created by the HCP-functional pre-processing pipeline.
%   The coordinate of the right corneal surface is also provided. The
%   motion of the head over time is then used to produce a vector that
%   specifies the relative displacement of the eye tracking camera with
%   respect to the corneal surface during a scan. This vector is
%   subsequently used in the analysis of eye tracking videos.
%
...
%       EPItoT1w.dat          - The EPI --> T1 registration
...
%       done using the freesurfer command vol2vol, along with the EPI Scout
%       image for the acquisition, the T1 volume that has the same
%       coordinates in which the eye coordinate was defined, and the
%       '_EPItoT1w.dat' file that defines the registration.
...
%       given in the Movement_Regressors.txt. There is much fussing with
%       the different coordinate frames. The movement of the eye coordinate
%       is made relative to the position of the eye at the first TR. As the
%       temporal resolution of the head motion regressors is far coarser
%       than the eye tracking video, a smooth interpolation of the head
%       motion is performed to create a vector that may be related to the
%       eye tracking videos.
...
mri = MRIread(escapeFileCharacters(p.Results.t1FileName));
...
% anterior-posterior, right-left, inferior-superior.
